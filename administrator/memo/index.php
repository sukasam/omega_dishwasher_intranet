...
if ($_GET['action'] == "delete") {
	...
	if ($code == "1") {
		...
		header("location:?cus_id=" . $_GET['cus_id']);
	}
}
...
if ($_GET['b'] <> "" and $_GET['s'] <> "") {
	...
	header("location:?cus_id=" . $_GET['cus_id'] . "&id=" . $_GET['id'] . $conpage);
}
...
<BODY>
	<DIV id=body-wrapper>
		...
		<DIV id=main-content>
			...
			<UL class=shortcut-buttons-set>
				<LI>
					...
					<A class=shortcut-button href="<?php echo $backLink; ?>"><SPAN><IMG alt=icon src="../images/btn_back.png"><BR>
							กลับ</SPAN></A></LI>
				<LI><A class=shortcut-button href="update.php?mode=add&cus_id=<?php echo $_GET['cus_id']; ?>"><SPAN><IMG alt=icon src="../images/pencil_48.png"><BR>
							เพิ่ม</SPAN></A></LI>
				<?php
				if ($FR_module <> "") {
			...
			<H3 align="left"><?php if ($_GET['tab'] == 2) {
									echo 'ใบเสนอราคาเช่า';
								} else if ($_GET['tab'] == 3) {
									echo 'เลขที่ First order';
								} else {
									echo 'หมายเลข';
								} ?> : <?php echo $quinfo['fs_id']; ?></H3><br>
				...
				<DIV class=content-box-header align="right" style="padding-right:15px;">
					...
					<div style="float:right;padding-top:10px;">
						<form name="form1" method="get" action="index.php">
							...
							?>
							<a href="index.php?id=<?php echo $_GET['id']; ?>">แสดงทั้งหมด</a>
							<?php
							/*$a_not_exists = array();
			post_param($a_param,$a_not_exists);*/
					...
					<div style="float:right;margin-right:20px;padding-top:10px;">
						...
						<select name="catalog_master" id="catalog_master" style="height:24px;" onChange="MM_jumpMenu('parent',this,0)">
							<option value="index.php?cus_id=<?php echo $_GET['cus_id']; ?>" <?php if (!isset($_GET['process'])) {
																								echo "selected";
																							} ?>>กรุณาเลือก</option>
							<option value="index.php?process=0&cus_id=<?php echo $_GET['cus_id']; ?>" <?php if ($_GET['process'] == '0') {
																											echo "selected";
																										} ?>>รอการแก้ไข</option>
							<option value="index.php?process=1&cus_id=<?php echo $_GET['cus_id']; ?>" <?php if ($_GET['process'] == '1') {
																											echo "selected";
																										} ?>>รอผู้อนุมัติฝ่ายขาย</option>
							<!--
         <option value="index.php?process=2&cus_id=<?php echo $_GET['cus_id']; ?>" <?php if ($_GET['process'] == '2') {
																						echo "selected";
																					} ?>>รอผู้อนุมัติฝ่ายการเงิน</option>-->
							<option value="index.php?process=3&cus_id=<?php echo $_GET['cus_id']; ?>" <?php if ($_GET['process'] == '3') {
																											echo "selected";
																										} ?>>รอผู้มีอำนาจลงนาม</option>
							<!--         <option value="index.php?process=4&cus_id=<?php echo $_GET['cus_id']; ?>" <?php if ($_GET['process'] == '4') {
																														echo "selected";
																													} ?>>รอผู้อนุมัติฝ่ายช่าง</option>-->
							<option value="index.php?process=5&cus_id=<?php echo $_GET['cus_id']; ?>" <?php if ($_GET['process'] == '5') {
																											echo "selected";
																										} ?>>ผ่านการอนุมัติ</option>
						</select>
					</div>
					<DIV class=clear>
						...
						<form name="form2" method="post" action="confirm.php" onSubmit="return check_select(this)">
							<TABLE>
								<THEAD>
									<TR>
										<TH width="10%">
											<div align="center"><a>การอนุมัติ</a></div>
										</TH>
										<TH width="15%">
											<div align="center"><a>Memo ID</a></div>
										</TH>
										<TH width="15%">
											<div align="center"><a>Memo โดย</a></div>
										</TH>
										<TH width="15%">
											<div align="center"><a>วันที่</a></div>
										</TH>
								...
								<TBODY>
									<?php
									if ($orderby == "") $orderby = "sm." . $PK_field;
									if ($sortby == "") $sortby = "DESC";

									$sql = "SELECT * FROM $tbl_name AS sm WHERE fo_id=" . $_GET['cus_id'];
									...
									while ($rec = @mysqli_fetch_array($query)) {
										$counter++;
										//						$row_sr2 = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM s_memo2 WHERE srid= '".$rec['id']."'"));
										...
										<TR>
											...
											<TD style="vertical-align:middle;">
												<center>
													...
													?>
														<select name="process_applove" style="background:#FFFFFF;color:#000;" onchange="selectProcess('<?php echo $rec['id']; ?>')" id="process_<?php echo $rec['id']; ?>">
															<option value="0" <?php if ($rec['process'] == '0') {
																					echo 'selected';
																				} ?>>รอแก้ไข</option>
											...
											<TD style="vertical-align:middle;"><?php $chaf = str_replace("/", "-", $rec["mo_id"]); ?><div align="center"><span class="text"><a href="../../upload/memo/<?php echo $chaf; ?>.pdf" target="_blank"><?php echo $rec["mo_id"]; ?></a></span></div>
											</TD>
											<TD style="vertical-align:middle;">
												<center><span class="text"><?php echo getsalename($conn, $rec["cs_sell"]); ?></span></center>
											</TD>
											<TD style="vertical-align:middle;">
												<center><span class="text"><?php echo format_date_th($rec["memo_open"], 2); ?></span></center>
											</TD>
											<td style="vertical-align: middle;">
												<center><a href="../document_memo/?fo_id=<?php  echo $rec[$PK_field];?>&target=MEMO&cus_id=<?php echo $_GET['cus_id'];?>"><img src="../images/document.png" height="30"></a></center>
											</td>
											<!--
          <TD style="vertical-align:middle"><div align="center">
            <?php if ($rec["st_setting"] == 0) { ?>
            <a href="../memo/?b=<?php echo $rec[$PK_field]; ?>&s=<?php echo $rec["st_setting"]; ?>&page=<?php echo $_GET['page']; ?>&id=<?php echo $rec["id"]; ?>&cus_id=<?php echo $_GET['cus_id']; ?>"><img src="../icons/status_on.gif" width="10" height="10"></a>
            <?php  } else { ?>
            <a href="../memo/?b=<?php echo $rec[$PK_field]; ?>&s=<?php echo $rec["st_setting"]; ?>&page=<?php echo $_GET['page']; ?>&id=<?php echo $rec["id"]; ?>&cus_id=<?php echo $_GET['cus_id']; ?>"><img src="../icons/status_off.gif" width="10" height="10"></a>
            <?php  } ?>
          </div></TD>
-->
											<TD style="vertical-align:middle;">
												<!-- Icons -->
												<div align="center"><A title=Edit href="update.php?mode=update&cus_id=<?php echo $_GET['cus_id']; ?>&id=<?php echo $rec[$PK_field]; ?>"><IMG src="../images/icons/paper_content_pencil_48.png" alt=Edit width="25" height="25" title="แก้ไขใบแจ้งงานบริการ"></A></div>
											</TD>
											<TD style="vertical-align:middle;">
												<div align="center"><A title="Delete" href="#"><IMG alt=Delete src="../images/cross.png" onClick="confirmDelete('?action=delete&id=<?php echo $rec["id"]; ?>&cus_id=<?php echo $_GET['cus_id']; ?>','Group  <?php echo $rec[$PK_field]; ?> : <?php echo $rec["mo_id"]; ?>')"></A></div>
											</TD>
										</TR>
									<?php  } ?>
								</TBODY>
