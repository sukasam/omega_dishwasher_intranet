...
if($_GET['action'] == 'getsn'){
		...
		$qusn1 = @mysqli_query($conn,"SELECT * FROM s_group_sn WHERE 1 ".$consd." ORDER BY group_id ASC");
...
if($_GET['action'] == 'getsnkey'){
		...
		//echo "SELECT group_name FROM s_group_typeproduct ".$consd." ORDER BY group_name ASC";
		$qu_cus = mysqli_query($conn,"SELECT * FROM s_group_sn ".$consd." ORDER BY group_id ASC");
		while($row_cus = @mysqli_fetch_array($qu_cus)){
