	...
	if ($_POST[mode] <> "") { 
		...
		if ($_POST[mode] == "add") { 
			...
			foreach($codes as $a => $b){
				//$BX_NAME[$a]
				@mysql_query("INSERT INTO `s_service_report3sub` (`r_id`, `sr_id`, `codes`, `lists`, `units`, `prices`, `amounts`, `opens`, `remains`) VALUES (NULL, '".$id."', '".$codes[$a]."', '".$lists[$a]."', '".$units[$a]."', '".$prices[$a]."', '".$amounts[$a]."', '".$opens[$a]."', '".$remains[$a]."');");
				$idsp = mysql_insert_id();
				@mysql_query("UPDATE `s_group_sparpart` SET `group_stock` = `group_stock` - '".$opens[$a]."' WHERE `group_id` = '".$lists[$a]."';");
				@mysql_query("UPDATE `s_service_report3sub` SET `amounts` = `amounts` - '".$opens[$a]."' WHERE `r_id` = '".$idsp."';");
			}
		...
		if ($_POST[mode] == "update" ) {
			...
			foreach($codes as $a => $b){
				$resupdate = get_plusminus("s_group_sparpart","s_service_report3sub",$rid[$a],$lists[$a]);
				
				@mysql_query("UPDATE `s_service_report3sub` SET `codes` = '".$codes[$a]."', `lists` = '".$lists[$a]."', `units` = '".$units[$a]."', `prices` = '".$prices[$a]."', `opens` = '".$opens[$a]."' WHERE `r_id` =".$rid[$a]."");
				@mysql_query("UPDATE `s_group_sparpart` SET `group_stock` = `group_stock` - '".$opens[$a]."' WHERE `group_id` = '".$lists[$a]."';");
				
				$amount = @mysql_fetch_array(@mysql_query("SELECT * FROM `s_group_sparpart` WHERE `group_id` = '".$lists[$a]."';"));
								
				@mysql_query("UPDATE `s_service_report3sub` SET `amounts` = '".$amount['group_stock']."' WHERE `r_id` = '".$rid[$a]."';");	
				
				if($opens[$a] == 0){
					@mysql_query("UPDATE `s_service_report3sub` SET `codes` = '', `lists` = '', `units` = '', `prices` = '', `amounts` = '', `opens` = '', `remains` = '' WHERE `r_id` =".$rid[$a]."");
				}
			}	
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="dataTable" style="text-align:center;margin-top:5px;">
      <tr>
       ...
       <!-- <td width="9%" style="border:1px solid #000000;font-size:12px;font-family:Verdana, Geneva, sans-serif;padding-top:10px;padding-bottom:10px;text-align:center;"><strong>จำนวนคงเหลือ</strong></td>-->
        ...
        <?php  
		 $qu = @mysql_query("SELECT * FROM s_service_report3sub WHERE sr_id = '".$sr_id."' ORDER BY r_id ASC");
