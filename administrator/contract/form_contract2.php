...
foreach ($_POST['chkTypePro'] as $key => $value) {
	...
	if ($value === 'เครื่องผลิตน้ำแข็ง') {
		$hrService = '72';
		$proCH = 1;
		$optionTypeContract = 'ผลิตน้ำแข็ง';
	} else if ($value === 'เครื่องล้างแก้ว') {
		$hrService = '48';
		$proCH = 0;
		$optionTypeContract = 'ล้างแก้ว';
	} else {
		$hrService = '48';
		$proCH = 0;
		$optionTypeContract = 'ล้างจาน';
	}

	$nameTypeContract = $value;
}
...
for ($i = 1; $i <= 7; $i++) {
	if ($finfos['cpro' . $i]) {
		if ($finfos['pro_pod' . $i] != "") {
			$propod = ' รุ่น ' . $finfos['pro_pod' . $i];
		} else {
			$propod = '';
		}
		if ($finfos['pro_sn' . $i] != "") {
			$prosn = ' S/N ' . $finfos['pro_sn' . $i];
		} else {
			$prosn = '';
		}

		$listPro .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2.' . $i . ' ' . get_proname($conn, $finfos['cpro' . $i]) . ' ' . get_prodetail($conn, $finfos['cpro' . $i]) . $propod . $prosn . ' จำนวน ' . $finfos['camount' . $i] . ' ' . get_pronamecall($conn, $finfos['cpro' . $i]) . '<br>';
		$runPro++;
	}
}
...
$form = '<style>
	...
	<div class="conID">
	 ...
	 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>'.$_POST['companyName'].'</strong> โดย <strong>'.$_POST['companyPerson'].'</strong> กรรมการผู้มีอำนาจ สำนักงานตั้งอยู่เลขที่ '.$_POST['companyAddress'].' เบอร์โทร '.$_POST['companyTelephone'].' ซึ่งต่อไปนี้สัญญานี้เรียกว่า “ ผู้ให้เช่า ” ฝ่ายหนึ่ง กับ<br>
	...
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ผู้เช่าตกลงเช่าและผู้ให้เช่าตกลงให้เช่า' . $nameConproCh . ' ของ '.$_POST['companyName'].' เพื่อใช้' . $text1 . 'ใน ' . $finfos['loc_name'] . ' ที่ตั้งร้านเลขที่ ' . $finfos['loc_address'] . '  โทรศัพท์ ' . $finfos['cd_tel'] . ' ซึ่งต่อไปในสัญญานี้เรียกว่า “' . $nameConpro . '” ตามรายละเอียดแนบท้ายสัญญา<br><br>
...
$form .= 'หมายเหตุ : อุปกรณ์เสริมที่ทางผู้ให้เช่าให้ผู้เช่ายืม (หากรายการอุปกรณ์ตามข้อ 2 ดังกล่าวมีความเสียหายใดๆ หรือมีการสูญหายเกิดขึ้น หรือเกิดจากอัคคีภัยหรือภัยธรรมชาติ ทางผู้เช่าจะเป็นผู้รับผิดชอบทั้งหมดทุกกรณี โดยไม่มีเงื่อนไข' . $txt2 . ')	<br><br>
	...
	<strong class="colorbBlue">(สัญญาเล่มนี้เป็นสัญญาปีที่ ' . $numContract . ' (ปีแรก ปี ' . $contractFY . ' เดือน ' . format_date_th($_POST['con_startdate'], 7) . ') ตามเลขที่สัญญาเล่มแรก ' . $contractFN . ')</strong><br>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>ตลอดระยะเวลาการเช่าและหลังครบกำหนดระยะเวลาการเช่า ผู้ให้เช่าเป็นผู้ครอบครองและเป็นเจ้าของ กรรมสิทธิ์ใน' . $nameTypeContract . 'และอุปกรณ์ประกอบต่าง ๆ ตามสัญญาข้อ 1. และ ข้อ 2. โดยผู้เช่าไม่มีสิทธิ์ โต้แย้งใด ๆ ทั้งสิ้น</strong>
	<br><br>
	...
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="colorRed">บริษัทฯ ไม่มีนโยบายให้พนักงานรับชำระค่าสินค้าและค่าบริการจากลูกค้าผ่านบัญชีธนาคารส่วนตัวของพนักงาน
	หากพบเหตุการณ์ดังกล่าว โปรดติดต่อเจ้าหน้าที่ของบริษัทฯโดยเร็วที่สุด โทร 08-2323-3535<br>
	กรุณาโอนเงินเข้าบัญชีธนาคาร “บริษัท โอเมก้า กรุ๊ป คอร์ปอเรชั่น จำกัด” เท่านั้น<br>
	บริษัท โอเมก้า กรุ๊ป คอร์ปอเรชั่น จำกัด ออมทรัพย์ เลขทีบัญชี 011-190420-0 ธนาคารกสิกร<br>
	บริษัท โอเมก้า กรุ๊ป คอร์ปอเรชั่น จำกัด ออมทรัพย์ เลขทีบัญชี 127-242699-9 ธนาคารไทยพาณิชย์<br>
	บริษัทฯ จะไม่รับผิดชอบต่อความเสียหาย/สูญเสียทีเกิดขึ้น ไม่ว่าจะกรณี ใดๆทั้งสิ้น</span><br><br>
	...
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;9.1  ผู้เช่าตกลงดูแลรักษาและใช้' . $nameConpro . 'ที่เช่าตามคำแนะนำของผู้ให้เช่าอย่างเคร่งครัด หากทรัพย์สิน ที่เช่าเสียหายเพราะการใช้งานนอกเหนือจากการ' . $optionTypeContract . 'ตามปกติที่ผู้ให้เช่า แนะนำไว้ผู้เช่าตกลงรับผิดชอบ ในความเสียหายทั้งหมด <br>
...
if ($proCH == 0) {
	...
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;11.1 ผู้เช่าตกลงใช้น้ำยาทำความสะอาดภาชนะสำหรับ' . $nameConpro . 'ของ บริษัท โอเมก้า กรุ๊ป คอร์ปอเรชั่น จำกัด กับ' . $nameConpro . 'ที่เช่าตามสัญญานี้<br>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" style="text-align:center;margin-top:5px;">
      <tr>
        ...
        <td width="50%" class="signature" style="vertical-align: top;">
        	<br><br>ในนาม <strong>'.$_POST['companyName'].'</strong><br><br><br><br>
