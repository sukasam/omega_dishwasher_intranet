...
<head>
	...
	<script type="text/javascript">
		function get_customer(cid, cname) {
			...
			self.opener.checkfirstorder(cid, 'cusadd', 'cusprovince', 'custel', 'cusfax', 'contactid', 'datef', 'datet', 'cscont', 'cstel', 'sloc_name', 'sevlast', 'prolist', 'ct_type2', 'loc_clean', 'loc_clean_sn');
			self.opener.document.getElementById("rsnameid").innerHTML = "<input type=\"hidden\" name=\"cus_id\" value=\"" + cid + "\">";
			var modeGet = self.opener.document.getElementById("mode").value;
			self.opener.document.getElementById("mode").value = 'c' + modeGet;
			self.opener.document.getElementById("form1").submit();
			window.close();
			//		if(loc_clean != ""){
			//			
			//		}
			//		self.opener.document.getElementById("loc_clean").value="ss";
			//		self.opener.document.getElementById("loc_clean_sn").value="565";
...
<body>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tv_search" id="rscus">
		...
		$qu_cus = mysqli_query($conn, "SELECT fo_id,cd_name,loc_name FROM s_first_order WHERE 1 ".$conDealer." ORDER BY cd_name ASC");
		while ($row_cus = @mysqli_fetch_array($qu_cus)) {
		?>
			<tr>
				<td><A href="javascript:void(0);" onclick="get_customer('<?php echo $row_cus['fo_id']; ?>','<?php echo $row_cus['cd_name']; ?>');"><?php echo $row_cus['cd_name']; ?> (<?php echo $row_cus['loc_name'] ?>)</A>
				</td>
			</tr>
		<?php
		}
