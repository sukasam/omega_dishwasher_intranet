...
<script type="text/javascript">
   function get_product(group_id,group_name,protype){
	//alert(group_id);
	var xmlHttp;
...
function get_pro2(pval,param){
	/*alert(pval);
	alert(param);*/
	var xmlHttp;
   xmlHttp=GetXmlHttpObject(); //Check Support Brownser
   URL = pathLocal+'ajax_return.php?action=getpro2&pval='+pval+'&keys='+param;
   if (xmlHttp==null){
      alert ("Browser does not support HTTP Request");
      return;
   }
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tv_search">
  <tr>
    <td colspan="2"><strong>ค้นหา&nbsp;&nbsp;:&nbsp;&nbsp;</strong>
        <input type="text" name="textfield" id="textfield" style="width:85%;" onkeyup="get_pro2(this.value,'<?php  echo $_GET['protype']?>');"/>
    </td>
  </tr>
</table>
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tv_search">
...
<?php  
  	$qu_cus = mysqli_query($conn,"SELECT * FROM s_group_typeproduct WHERE 1=1 AND group_spro_id LIKE '04-%' ORDER BY group_name ASC");
