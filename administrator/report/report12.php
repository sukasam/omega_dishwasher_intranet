...
<title>เลือกตามชื่อร้าน-ชื่อบริษัท ( <?php  if($cd_name != ""){echo $cd_name;}else{echo "ทั้งหมด";}?> )</title>
<style type="text/css">
 .tbreport{
 	font-size:10px;
 }
...
<body>
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tbreport">
	  <tr>
	    <th colspan="2" style="text-align:left;font-size:12px;"><p>บริษัท โอเมก้า ดิชวอชเชอร์ (ประเทศไทย) จำกัด<br />
	      รายงานสรุปเงินประกันเครื่องเช่า ( <?php  if($cd_name != ""){echo $cd_name;}else{echo "ทั้งหมด";}?> )</p></th>
	    <th colspan="6" style="text-align:right;font-size:11px;"><?php  echo $dateshow;?></th>
      </tr>
      <tr>
        <?php  if($_REQUEST['sh1'] == 1){?><th width="14%">ชื่อลูกค้า / บริษัท + เบอร์โทร</th><?php  }?>
        <?php  if($_REQUEST['sh2'] == 1){?><th width="20%">ชื่อร้าน / สถานที่ติดตั้ง</th><?php  }?>
        <?php  if($_REQUEST['sh3'] == 1){?><th width="8%">จังหวัด</th><?php  }?>
        <?php  if($_REQUEST['sh4'] == 1){?><th width="8%">ประเภทลูกค้า</th><?php  }?>
        ...
        <th width="8%">เงินประกัน</th><?php  }?>
      </tr>
      <?php  
		$sql = "SELECT * FROM s_first_order WHERE 1 ".$daterriod." ".$condition." ".$codi." ORDER BY date_forder ASC";
	  ...
	  ?>
      <tr>
		<td colspan="8" style="text-align:right;"> <strong>รวมยอดเงินประกันทั้งสิ้น&nbsp;&nbsp;<?php  echo number_format($sumMnGar);?>&nbsp;&nbsp;บาท&nbsp;&nbsp;</strong></td>
	  </tr>
       <tr>
