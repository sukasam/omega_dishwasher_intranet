...
function get_pro(pval,param){
	/*alert(pval);
	alert(param);*/
	var xmlHttp;
   xmlHttp=GetXmlHttpObject(); //Check Support Brownser
   URL = pathLocal+'ajax_return.php?action=getpro&pval='+pval+'&keys='+param;
   if (xmlHttp==null){
      alert ("Browser does not support HTTP Request");
      return;
   }
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tv_search">
  <tr>
    <td colspan="2"><strong>ค้นหา&nbsp;&nbsp;:&nbsp;&nbsp;</strong>
        <input type="text" name="textfield" id="textfield" style="width:85%;" onkeyup="get_pro(this.value,'<?php  echo $_GET['protype']?>');"/>
    </td>
  </tr>
</table>
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tv_search">
...
<?php  
  	$qu_cus = mysql_query("SELECT * FROM s_group_typeproduct ORDER BY group_name ASC");
