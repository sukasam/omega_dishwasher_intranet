...
if ($_GET['action'] === 'getSparScan') {
	if (!empty($_GET['scan_part'])) {
		$quQry = mysqli_query($conn, "SELECT * FROM `s_service_report2sub` WHERE sr_id = '" . $_GET['scan_part'] . "' ORDER BY r_id ASC");
		$data = array();
		while ($rowPro = mysqli_fetch_array($quQry, MYSQLI_ASSOC)) {
			array_push($data, array('group_id' => $rowPro['lists'], 'group_spar_barcode' => get_sparpart_barcode($conn, $rowPro['lists']), 'group_name' => get_sparpart_name($conn, $rowPro['lists']), 'group_namecall' => $rowPro['units'], 'group_stock' => getStockSpar($conn, $rowPro['lists']), 'group_qty' => intval($rowPro['opens'])));
		}
