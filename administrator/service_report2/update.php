<?php 
	...
	if ($_POST[mode] <> "") { 
		...
		if ($_POST[mode] == "add") { 
			...
			@mysql_query("UPDATE `s_service_report` SET `cpro1` = '".$_POST['lists'][0]."', `cpro2` = '".$_POST['lists'][1]."', `cpro3` = '".$_POST['lists'][2]."', `cpro4` = '".$_POST['lists'][3]."', `cpro5` = '".$_POST['lists'][4]."', `camount1` = '".$_POST['opens'][0]."', `camount2` = '".$_POST['opens'][1]."', `camount3` = '".$_POST['opens'][2]."', `camount4` = '".$_POST['opens'][3]."', `camount5` = '".$_POST['opens'][4]."', `cprice1` = '".$_POST['prices'][0]."', `cprice2` = '".$_POST['prices'][1]."', `cprice3` = '".$_POST['prices'][2]."', `cprice4` = '".$_POST['prices'][3]."', `cprice5` = '".$_POST['prices'][4]."' WHERE `sv_id` = '".$_POST['srid']."' LIMIT 1;");
			
			foreach($codes as $a => $b){
				//$BX_NAME[$a]
				@mysql_query("INSERT INTO `s_service_report2sub` (`r_id`, `sr_id`, `codes`, `lists`, `units`, `prices`, `amounts`, `opens`, `remains`) VALUES (NULL, '".$id."', '".$codes[$a]."', '".$lists[$a]."', '".$units[$a]."', '".$prices[$a]."', '".$amounts[$a]."', '".$opens[$a]."', '".$remains[$a]."');");
				$idsp = mysql_insert_id();
				@mysql_query("UPDATE `s_group_sparpart` SET `group_stock` = `group_stock` - '".$opens[$a]."' WHERE `group_id` = '".$lists[$a]."';");
				@mysql_query("UPDATE `s_service_report2sub` SET `amounts` = `amounts` - '".$opens[$a]."' WHERE `r_id` = '".$idsp."';");
			}
		...
		if ($_POST[mode] == "update" ) {
			...
			foreach($codes as $a => $b){
				...
				$resupdate = get_plusminus("s_group_sparpart","s_service_report2sub",$rid[$a],$lists[$a]);
				
				@mysql_query("UPDATE `s_service_report2sub` SET `codes` = '".$codes[$a]."', `lists` = '".$lists[$a]."', `units` = '".$units[$a]."', `prices` = '".$prices[$a]."', `opens` = '".$opens[$a]."' WHERE `r_id` =".$rid[$a]."");
				
				@mysql_query("UPDATE `s_group_sparpart` SET `group_stock` = `group_stock` - '".$opens[$a]."' WHERE `group_id` = '".$lists[$a]."';");
				
				$amount = @mysql_fetch_array(@mysql_query("SELECT * FROM `s_group_sparpart` WHERE `group_id` = '".$lists[$a]."';"));
								
				@mysql_query("UPDATE `s_service_report2sub` SET `amounts` = '".$amount['group_stock']."' WHERE `r_id` = '".$rid[$a]."';");	
				
				if($opens[$a] == 0){
					@mysql_query("UPDATE `s_service_report2sub` SET `codes` = '', `lists` = '', `units` = '', `prices` = '', `amounts` = '', `opens` = '', `remains` = '' WHERE `r_id` =".$rid[$a]."");
				}
	...
	if ($_GET[mode] == "add") { 
		 ...
		 $rowSR = mysql_fetch_array(mysql_query("SELECT * FROM s_service_report WHERE sr_id = '".$_GET['srid']."'"));
		 $finfo = get_firstorder($rowSR['cus_id']);
		 
		 $rowSR3 = mysql_fetch_array(mysql_query("SELECT * FROM s_service_report3 WHERE srid = '".$_GET['srid']."'"));
		 $srid2 = $rowSR3['sv_id'];
		//echo "MKUNG=".$srid2;
            	...
            	<!--<select name="cus_id" id="cus_id" onChange="checkfirstorder(this.value,'cusadd','cusprovince','custel','cusfax','contactid','datef','datet','cscont','cstel','sloc_name','sevlast','prolist');" style="width:300px;">
                	...
                	<?php  
						...
						while($row_cusf = @mysql_fetch_array($qu_cusf)){
							?>
							<option value="<?php  echo $row_cusf['fo_id'];?>" <?php  if($row_cusf['fo_id'] == $rowSR['cus_id']){echo 'selected';}?>><?php  echo $row_cusf['cd_name']." (".$row_cusf['loc_name'].")";?></option>
							<?php 
						}
					?>
                </select>-->
                <input name="cd_names" type="text" id="cd_names"  value="<?php  echo get_customername($rowSR['cus_id']);?>" style="width:100%;" readonly>
...
<span class="inpfoder"><?php  echo $rowSR['sv_id'];?></span>
          ...
          <tr>
            ...
            <td><!--<strong>บริการครั้งล่าสุด : </strong> <span id="sevlast"><?php  echo get_lastservice_f($cus_id,$sv_id);?></span> &nbsp;&nbsp;&nbsp;&nbsp;--><strong>กำหนดคืนอะไหล่ :</strong> <span id="datet"></span>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="dataTable" style="text-align:center;margin-top:5px;">
      <tr>
        ...
        <?php  
		 $qu = @mysql_query("SELECT * FROM s_service_report2sub WHERE sr_id = '".$sr_id."' ORDER BY r_id ASC");
