<?php 
	...
	if($_GET[action] == "delete"){
		...
		if ($code == "1") {
			...
			unlink("../../upload/database/".$_GET['fn']);
			@mysql_query($sql);			
  ...
  <form name="form2" method="post" action="confirm.php" onSubmit="return check_select(this)">
    <TABLE>
      <THEAD>
        <TR>
          ...
          <TH width="7%" <?php  Show_Sort_bg ("user_id", $orderby) ?>> <?php 
	...
	?>
          ...
          <TH width="19%" <?php  Show_Sort_bg ("group_name", $orderby) ?>>
           <?php   Show_Sort_new ("group_name", "ชื่อไฟล์", $orderby, $sortby,$page,$param2);?>
            &nbsp;</TH>
          <TH width="64%" <?php  Show_Sort_bg ("group_name", $orderby) ?>> <?php   Show_Sort_new ("group_name", "ดาวน์โหลดไฟล์", $orderby, $sortby,$page,$param2);?>
            &nbsp;</TH>
      ...
      <TBODY>
        ...
        <TR>
          ...
          <TD><span class="text"><a href="../../upload/database/<?php  echo $rec["group_file"] ; ?>" target="_blank"><?php  echo $rec["group_file"] ; ?></a></span></TD>
          <!--<TD>
          ...
          <TD><A title=Delete  href="#"><IMG alt=Delete src="../images/cross.png" onClick="confirmDelete('?action=delete&<?php  echo $PK_field; ?>=<?php  echo $rec[$PK_field];?>&fn=<?php  echo $rec["group_file"];?>','Group  <?php  echo $rec[$PK_field];?> : <?php  echo $rec["group_name"];?>')"></A></TD>
        </TR>  
		<?php  }?>
      </TBODY>
    </TABLE>
