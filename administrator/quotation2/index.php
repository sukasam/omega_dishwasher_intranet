<?php 
	...
	if($_GET['action'] == "createQA"){
		...
		mysql_query("UPDATE `s_quotation2` SET `quotation` = '".$idQa."' WHERE `qu_id` = ".$_GET['id'].";");
		mysql_query("UPDATE `s_quotation` SET `quotation` = '".$_GET['id']."' WHERE `qu_id` = ".$idQa.";");
        ...
        <TR>
          ...
          <TD><span class="text"><?php  echo $rec["cd_name"] ; ?></span></TD>
					<TD style="text-align: center;"><?php
						if($rec["quotation"] != ""){
							$chafQA = eregi_replace("/","-",getQaBNumber($rec["quotation"]))
							?>
							<a href="../quotation/update.php?mode=update&qu_id=<?php echo $rec["quotation"];?>"><?php echo getQaBNumber($rec["quotation"]);?></a>
							<?php 
