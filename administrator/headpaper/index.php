...
if ($_GET['action'] == "delete") {
	...
	if ($code == "1") {
		...
		unlink('../../upload/headerpaper/'.$_GET['img']);
	...
	<DIV id=body-wrapper>
		...
		<DIV id=main-content>
			...
			<UL class=shortcut-buttons-set>
				<LI><A class=shortcut-button href="update.php?mode=add&user_id=<?php echo $userInfo['user_id'];?><?php if ($param <> "") echo "&" . $param; ?>"><SPAN><IMG alt=icon src="../images/pencil_48.png"><BR>
							เพิ่ม</SPAN></A></LI>
				<?php
				if ($FR_module <> "") {
					...
					<DIV id=tab1 class="tab-content default-tab">
						...
						<form name="form2" method="post" action="confirm.php" onSubmit="return check_select(this)">
							<TABLE>
								<THEAD>
									<TR>
										...
										<TH width="10%" <?php Show_Sort_bg("user_id", $orderby) ?>> <?php
											...
											&nbsp;</TH>
										<TH width="15%" <?php Show_Sort_bg("key_head", $orderby) ?>>
											<?php Show_Sort_new("key_head", "ชื่อหัวกระดาษ", $orderby, $sortby, $page, $param2); ?>
											&nbsp;</TH>
										<TH width="50%" <?php Show_Sort_bg("u_images", $orderby) ?>> <?php Show_Sort_new("u_images", "รูปหัวกระดาษ", $orderby, $sortby, $page, $param2); ?>
											&nbsp;</TH>
										<TH width="10%"><a>แก้ไข</a></TH>
										<TH width="10%"><a>ลบ</a></TH>
									</TR>
								</THEAD>
								<TFOOT>
								</TFOOT>
								<TBODY>
									<?php
									if ($orderby == "") $orderby = $tbl_name . ".group_id";
									...
									$sql.= ' and user_id='.$userInfo['user_id'];
									...
									?>
										<TR>
											...
											<TD><span class="text"><?php echo get_header_paper($conn,$rec["key_head"]); ?></span></TD>
											<TD><span class="text"><img src="../../upload/headerpaper/<?php echo $rec["u_images"]; ?>" height="100"></span></TD>
											<TD>
												<!-- Icons -->
												<A title=Edit href="update.php?mode=update&user_id=<?php echo $userInfo['user_id'];?>&<?php echo $PK_field; ?>=<?php echo $rec[$PK_field];
																														if ($param <> "") { ?>&<?php echo $param;
																																									} ?>"><IMG alt=Edit src="../images/pencil.png"></A> <A title=Delete href="#"></A></TD>
											<TD><A title=Delete href="#"><IMG alt=Delete src="../images/cross.png" onClick="confirmDelete('?action=delete&user_id=<?php echo $userInfo['user_id'];?>&img=<?php echo $rec["u_images"]; ?>&<?php echo $PK_field; ?>=<?php echo $rec[$PK_field]; ?>','Group  <?php echo $rec[$PK_field]; ?> : <?php echo $rec["group_name"]; ?>')"></A></TD>
										</TR>
									<?php  } ?>
								</TBODY>
							</TABLE>
