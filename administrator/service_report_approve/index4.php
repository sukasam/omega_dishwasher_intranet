...
<div style="float:right;padding-top:5px;">
	<form name="form1" method="get" action="index4.php">
...
<div style="float:right;margin-right:20px;padding-top:5px;">  
    ...
    <select name="catalog_master" id="catalog_master" style="height:24px;" onChange="MM_jumpMenu('parent',this,0)">
		<option value="index4.php?app_id=0" <?php  if($_GET['app_id'] == 0){echo "selected";}?>>รอการอนุมัติ</option>
		 <option value="index4.php?app_id=1" <?php  if($_GET['app_id'] == 1){echo "selected";}?>>อนุมัติ</option>
         <option value="index4.php?app_id=2" <?php  if($_GET['app_id'] == 2){echo "selected";}?>>ไม่อนุมัติ</option>
         <!--<option value="index4.php?app_id=3" <?php  if($_GET['app_id'] == 3){echo "selected";}?>>จ่ายแล้ว</option>-->
  	</select>
    </div>
<DIV class=clear>
...
<DIV id=tab1 class="tab-content default-tab"><!-- This is the target div. id must match the href of this div's tab -->
  <form name="form2" method="post" action="confirm.php" onSubmit="return check_select(this)">
	...
	?>
            ...
            &nbsp;</TH>
          <TH width="9%"><div align="center"><a>IR ID</a></div></TH>
          ...
          <TH width="9%"><div align="center"><a>ดาวโหลด</a></div></TH>
          </TR>
      </THEAD>
      <TFOOT>
        </TFOOT>
      <TBODY>
        ...
        <TR>
          ...
          <TD style="vertical-align:middle"><select name="jumpMenu" id="jumpMenu" onChange="MM_jumpMenu('parent',this,0)" style=" <?php  if($rec["approve"] == "0"){echo "background:#FF0;color:#000;";}elseif($rec["approve"] == "1"){echo "background:#090;color:#FFF;";}elseif($rec["approve"] == "3"){echo "background:#C60;color:#FFF;";}else{echo "background:#F00;color:#FFF;";}?>">
            <option value="index4.php?action=apps&cc=0&sr_id=<?php  echo $rec["sr_id"];?>&page=<?php  echo $_GET['page'];?>" <?php  if($rec["approve"] == "0"){echo 'selected="selected"';}?> style="background:#FFF;color:#000;">รอการอนุมัติ</option>
            <option value="index4.php?action=apps&cc=1&sr_id=<?php  echo $rec["sr_id"];?>&page=<?php  echo $_GET['page'];?>" <?php  if($rec["approve"] == "1"){echo 'selected="selected"';}?> style="background:#FFF;color:#000;">อนุมัติ</option>
            <option value="index4.php?action=apps&cc=2&sr_id=<?php  echo $rec["sr_id"];?>&page=<?php  echo $_GET['page'];?>" <?php  if($rec["approve"] == "2"){echo 'selected="selected"';}?> style="background:#FFF;color:#000;">ไม่อนุมัติ</option>
           <!-- <option value="index4.php?action=apps&cc=3&sr_id=<?php  echo $rec["sr_id"];?>&page=<?php  echo $_GET['page'];?>" <?php  if($rec["approve"] == "3"){echo 'selected="selected"';}?> style="background:#FFF;color:#000;">จ่ายแล้ว</option>-->
          </select></TD>
