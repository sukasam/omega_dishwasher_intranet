<?php 
	...
	if($_GET['action'] == 'getprodetail'){
			...
			$prolistname = get_profirstorder($fid);
			$prolistpod = get_podfirstorder($fid);
			$prolistsn = get_snfirstorder($fid);
			
			echo '<input type="text" name="loc_pro" value="'.get_proname($prolistname[$pid]).'" id="loc_pro" class="inpfoder" style="width:50%;">|<input type="text" name="loc_seal" value="'.$prolistpod[$pid].'" id="loc_seal" class="inpfoder" style="width:20%;">|<input type="text" name="loc_sn" value="'.$prolistsn[$pid].'" id="loc_sn" class="inpfoder" style="width:20%;">';
	}
	
		if($_GET['action'] == 'getcusfirsh'){
		$fpid = $_GET['pid'];
		$rowcus  = @mysql_fetch_array(@mysql_query("SELECT * FROM s_first_order WHERE fo_id  = '".$fpid."'"));
		
		$prolist = get_profirstorder($fpid);
		...
		$displ = $rowcus['cd_address']."|".province_name($rowcus['cd_province'])."|".$rowcus['cd_tel']."|".$rowcus['cd_fax']."|".$rowcus['fs_id']."|".format_date($rowcus['date_quf'])."||".$rowcus['c_contact']."|".$rowcus['c_tel']."|".$rowcus['loc_name']."|".get_lastservice_s($fpid,"").'|'.$plid.'|'.$ctyp;
		echo $displ;
	}
	...
	if($_GET['action'] == 'getcus'){
		...
		$qu_cus = mysql_query("SELECT fo_id,cd_name,loc_name FROM s_first_order ".$consd." ORDER BY cd_name ASC");
		while($row_cusx = @mysql_fetch_array($qu_cus)){
			?>
			 <tr>
	...
	if($_GET['action'] == 'getsparpart'){
		...
		$qu_cus = mysql_query("SELECT * FROM s_group_sparpart ".$consd." ORDER BY group_spar_id ASC");
		while($row_cusx = @mysql_fetch_array($qu_cus)){
			?>
			 <tr>
	...
	if($_GET['action'] == 'getsparactive'){
		...
		$qu_spare = @mysql_query("SELECT * FROM s_group_sparpart  WHERE  group_id = '".$sparval."'");
		$row_spare = @mysql_fetch_array($qu_spare);
		...
		$selclist = "<select name=\"lists[]\" id=\"lists".$ressdata."\" class=\"inputselect\" style=\"width:92%\" onchange=\"showspare(this.value,'codes".$ressdata."','units".$ressdata."','prices".$ressdata."','amounts".$ressdata."')\">";
					...
					while($row_spares = @mysql_fetch_array($qucgspare)){
