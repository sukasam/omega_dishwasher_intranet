...
$(document).ready(function() {
	$.ajax({
		type: "GET",
		url: "call_return.php?action=getProScan&scan_part=" + getUrlVars()["sub_id"],
		success: function(data) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (obj.status === 'yes') {
						...
						var foundValue = proPartList.filter(findSpar => findSpar.group_spar_barcode === obj.group_spar_barcode);
						...
						for (i = 0; i < proPartList.length; i++) {
							if (proPartList[i].group_qty >= 1) {
								tableProList += '<tr>';
								tableProList += '<td style="border:1px solid #000000;padding:5;text-align:center;">' + parseInt(i + 1) + '<input type="hidden" name="cpro[]" value="' + proPartList[i]['group_id'] + '" id="cpro' + proPartList[i]['group_id'] + '" class="inpfoder" style="width:100%;text-align:center;"></td>';
								tableProList += '<td style="border:1px solid #000000;padding:5;text-align:center;">' + proPartList[i]['group_spar_barcode'] + '<input type="hidden" name="barcode[]" value="' + proPartList[i]['group_spar_barcode'] + '" id="barcode' + proPartList[i]['group_id'] + '" class="inpfoder" style="width:100%;text-align:center;"></td>';
								tableProList += '<td style="border:1px solid #000000;text-align:left;padding:5;">' + proPartList[i]['group_name'] + '</td>';
								tableProList += '<td style="border:1px solid #000000;text-align:left;padding:5;">' + proPartList[i]['group_pro_pod_name'] + '<input type="hidden" name="cpod[]" value="' + proPartList[i]['group_pro_pod'] + '" id="cpod' + proPartList[i]['group_id'] + '" class="inpfoder" style="width:100%;text-align:center;"></td>';
								tableProList += '<td style="border:1px solid #000000;padding:5;text-align:center;"><input type="text" name="camount[]" value="' + proPartList[i]['group_qty'] + '" id="camount' + proPartList[i]['group_id'] + '" class="inpfoder" style="width:100%;text-align:center;" onkeypress="return isNumberKey(event)" onchange="changeAmount(' + proPartList[i]['group_id'] + ')"></td>';
								tableProList += '</tr>';
							}

						}
						$("#msg_scan").html('');
						$("#exp").html(tableProList);
						console.log(JSON.stringify(proPartList))
					} else {
...
<UL class=shortcut-buttons-set>
  ...
  กลับ</SPAN></A></LI>
  <LI><A class=shortcut-button href="../group_typeproduct_stockin/update.php?mode=add"><SPAN><IMG  alt=icon src="../images/icons/icon-48-info.png" width="31px"><BR>
  เวอร์ชั่นเก่า</SPAN></A></LI>
</UL>
<!-- End .clear -->
			...
			<tr>
				...
				<td>
					<table class="formFields" cellspacing="0" width="100%">
					  ...
					  <tr>
							<td colspan="2"><strong>แสกนรหัสสินค้า: </strong><input name="scan_part" type="text" id="scan_part" value="" size="30"> <span id="msg_scan" style="color:red;font-weight: bold;"></span></td>
					  </tr>
				  </table>
				</td>
			</tr>
		...
		if($_GET['mode'] == "add"){
			...
			while($rowPro = mysqli_fetch_array($quQry)){
				...
				<tr>
					...
					<td style="border:1px solid #000000;padding:5;text-align:center;">
						<?php echo get_probarcode($conn, $rowPro['sparpart_id']); ?>
					</td>
					<td style="border:1px solid #000000;text-align:left;padding:5;">
						<?php echo get_product_name($conn, $rowPro['sparpart_id']); ?>
					</td>
					<td style="border:1px solid #000000;text-align:left;padding:5;">
						<?php echo getpod_name($conn, $rowPro['sparpart_pod']); ?>
						<input type="hidden" name="cpod[]" value="<?php echo $rowPro['sparpart_pod']; ?>" id="cpod<?php echo $rowPro['sparpart_id']; ?>" class="inpfoder" style="width:100%;text-align:center;">
					</td>
					<td style="border:1px solid #000000;padding:5;text-align:center;">
