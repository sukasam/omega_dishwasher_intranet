<?php    
	...
	if ($_POST["mode"] <> "") { 
		...
		if($_POST['group_spro_id'] == ""){
			$_POST['group_spro_id'] = $_POST['group_spro_id2'];
		}
...
$( document ).ready(function() {
	...
	$( "#group_spro_id" ).blur(function() {
		
		var group_spro_id = $("#group_spro_id").val();
		var group_spro_id2 = $("#group_spro_id2").val();
		
		if(group_spro_id){
			
			if(!$('#edit_spar_id').is(':checked')){
				
				$("#group_spro_id")[0].disabled = true;
				...
				$.ajax({
					type: "GET",
					url: "call_return.php?action=chkProID&group_spro_id="+group_spro_id,
					success: function(data){
						...
						if(obj.status === 'yes'){
							...
							$("#mode").val('update');
							$("#group_spro_id2").val(obj.group_spro_id);
							...
							$("#group_location").val('');
							$("#group_namecall").val('');
							$("#group_spro_id2").val(group_spro_id);
				...
				//console.log(group_spro_id);
				//console.log(group_spro_id2);
				if(group_spro_id != group_spro_id2 ){
					$.ajax({
						type: "GET",
						url: "call_return.php?action=chkProID&group_spro_id="+group_spro_id,
						success: function(data){
	...
	$( "#edit_spar_id" ).change(function() {
	  //$( "#log" ).html( $( "input:checked" ).val() + " is checked!" );
		...
		if(chkBox){
			$("#group_spro_id")[0].disabled = false;
		}else{
			$("#group_spro_id")[0].disabled = true;
		}
	});
            ...
            <td><table class="formFields" cellspacing="0" width="100%">
              <tr >
                <td nowrap class="name">รหัสสินค้า</td>
				  <td><span id="chkDupID" class="hide" style="color: red;">รหัสสินค้าซ้ำ<br></span><input name="group_spro_id" type="text" id="group_spro_id"  value="<?php     echo $group_spro_id; ?>" size="60"> <span class="editIDPro hide"><input type="checkbox" name="edit_spar_id" id="edit_spar_id" value="1" disabled> แก้ไขรหัสสินค้า</span>
                <input name="group_spro_id2" type="hidden" id="group_spro_id2"  value="" size="60">
                </td>
              </tr>
              <tr >
